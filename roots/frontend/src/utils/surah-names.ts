...
/** Verse counts per surah (index 0 unused). Static — verified against
 *  the DB with `SELECT chapter, MAX(verse) FROM verses GROUP BY chapter`. */
...
export function getSurahMaxAyah(surah: number): number {
  return surah >= 1 && surah < SURAH_VERSE_COUNTS.length
    ? SURAH_VERSE_COUNTS[surah]
    : 0;
}
