...
def _build_user_prompt(payload: dict) -> str:
    ...
    if vtype == "word_origins":
        ...
        body = (
            ...
            "  hook        (~8s, ≤22 words): A YouTube-style thumb-stop opener. DO NOT deliver the contrast yet (the slide will land it visually a few seconds later). Instead, set curiosity. Pick one of these registers and write naturally — do NOT use these as fill-in-the-blank templates:\n"
            "                • A question that names the lens word or the lens move WITHOUT giving away the reveal. E.g. 'Why does the Quran's word for sin literally come from the word for tail?' / 'What if this verse isn't saying what your translation says?'\n"
            "                • A tease that signals 'one word changes this verse' without disclosing which way. E.g. 'There's one word in this verse that most English Qur'ans get wrong.' / 'A four-letter root flips the meaning of this verse.'\n"
            "                • A reframe of a familiar verse the audience thinks they know. E.g. 'Everyone reads this verse as a warning. The Arabic reads it as something more intimate.'\n"
            "             AVOID OPENERS THAT DUMP THE CONTRAST: do NOT start with 'Most translations say X but the Arabic says Y' — that's the slide's job, not the hook's. The hook earns the click; the verse_intro and insight earn the meaning. Keep the hook tight, curiosity-led, and verse-specific. Do not be preachy.\n"
