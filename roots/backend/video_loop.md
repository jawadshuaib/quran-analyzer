...
- Wakeup pacing (operator directives 2026-07-10): NO daily count ceiling —
  the loop runs until the sources are exhausted. ONE banking tick
  (mine+rate+draft+panel) per ~10 minutes (operator raised the cadence
  from 45-60 min on 2026-07-10; token burn accepted). Backpressure
  unchanged and primary: ≥10 gate_passed awaiting review → pull+distill
  only, sleep 60+ min. EXHAUSTION: when all four miners return zero new candidates
